...
class Lorentzian : public Oscillator
{
private:
	Field P;
...
public:
	...
	Lorentzian(Grid grid, u64 N, f32 omega_P_wl, f32 omega_0_wl, f32 gamma_wl, string name): 
		Oscillator(Type::LORENTZIAN),
		grid(grid),
		N(N),
		...
		b0(gamma),
		b1(omega_02),
		beta(1.0f/(1.0f + grid.dt * 0.5f * b1)),
		f2((4.0f + 2.0f * b0 * grid.dt + grid.dt2 * b1)),
		f1(2.0f * (4.0f - grid.dt2 * b1) / f2),
		f0((2.0f * b0 * grid.dt - grid.dt2 * b1  - 4.0f) / f2),
		d2((grid.dt2 * a0) / f2),
		d1((2.0f * grid.dt2 * a0) / f2),
		d0((grid.dt2 * a0) / f2),
	...
	Lorentzian* clone()
	{
		return new Lorentzian(grid, N, omega_P_wl, omega_0_wl, gamma_wl, name);
	}
